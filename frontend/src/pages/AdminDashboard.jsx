    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Review the application, update status, and send feedback to the
            user.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-[#faf8f5] rounded-xl p-4 text-sm space-y-1.5">
            <p className="font-semibold text-[#1e3a5f] text-base mb-2">
              Personal Details
            </p>
            ...
            <p className="text-gray-600">
              ...
              {app.created_at ? format(new Date(app.created_at), "PPP") : "—"}
            </p>
          </div>
          ...
          <div className="bg-[#faf8f5] rounded-xl p-4 text-sm space-y-1.5">
            ...
            {app.education_level && (
              <p className="text-gray-600">
                ...
                {EDUCATION_LABELS[app.education_level] || app.education_level}
              </p>
            )}
            {app.preferred_start_date && (
              <p className="text-gray-600">
                <span className="font-medium">Start Date:</span>{" "}
                {format(new Date(app.preferred_start_date), "PP")}
              </p>
            )}
            {app.message && (
              <p className="text-gray-600">
                <span className="font-medium">Notes:</span> {app.message}
              </p>
            )}
          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {app.documents.map((doc) => (
                  <div
                    key={doc.id}
                    className="border rounded-xl p-3 bg-[#faf8f5] w-44"
                  >
                    <p className="font-medium text-xs mb-2 truncate">
                      {doc.title}
                    </p>
                    {doc.file &&
                    ...
                    <a
                      href={doc.file}
                      download
                      className="block text-xs text-gray-400 mt-1 underline"
                    >
                      Download
                    </a>
                  </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400">
                Visible to applicant on their dashboard
              </span>
            </div>
            <Textarea
              value={feedback}
            ...
            {feedback && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-700">
                <strong>Preview:</strong> The user will see this message
                highlighted on their dashboard and will be prompted to resubmit
                if needed.
              </div>
            )}
          </div>
...
export default function AdminDashboard() {
  ...
  const loadStats = async () => {
    try {
      const r = await adminApi.getAppointmentStats();
      setStats(r.data);
    } catch {}
  };
  const loadAppointments = async () => {
    try {
      const r = await adminApi.getAllAppointments();
      const d = r.data;
  ...
  const loadMessages = async () => {
    try {
      ...
      setUnreadCount(c.data.unread_count);
    } catch {}
  };
  const loadApplicants = async () => {
    try {
      const r = await adminApi.getAllApplicants();
      const d = r.data;
      setApplicants(Array.isArray(d) ? d : d.results || []);
    } catch {}
  };
  const loadSlots = async () => {
    try {
      const r = await adminApi.getAllSlots();
      const d = r.data;
  ...
  const handleUpdatePaymentStatus = async (applicantId, newStatus) => {
    try {
      await adminApi.updateApplicantStatus(applicantId, {
  ...
  const handleUpdateAppointmentStatus = async (id, s) => {
    try {
      await adminApi.updateAppointmentStatus(id, s);
  ...
  const paidCount = applicants.filter(
    (a) => a.payment_status === "paid",
  ).length;
  const unpaidCount = applicants.filter(
    (a) => a.payment_status !== "paid",
  ).length;
